        ...
        for item in suggestions:
            for key, value in item.items():
                if isinstance(value, np.generic):
                    item[key] = value.item()  # Converts np.int64, np.float64 etc. to int, float
                elif isinstance(value, float) and (np.isnan(value)):
                    item[key] = None  # Handle NaNs
        return suggestions
...
def categorize_meal_type(cuisine_path):
    path = cuisine_path.lower()
    
    # Handle "soup" specifically
    if 'soup' in path:
        if any(keyword in path for keyword in ['main dish', 'meat', 'seafood', 'dinner', 'main dishes']):
            return 'dinner'
        elif any(keyword in path for keyword in ['breakfast', 'brunch']):
            return 'breakfast'
        else:
            return 'dinner'  # default to dinner

    # Check for breakfast-related items
    if any(keyword in path for keyword in ['breakfast', 'brunch', 'bread', 'pork']):
        return 'breakfast'
    
    # Check for dinner-related items
    elif any(keyword in path for keyword in ['main dish', 'meat', 'seafood']):
        return 'dinner'
    ...
    # Check for snacks
    elif any(keyword in path for keyword in ['snack', 'appetizer', 'side dish', 'desserts', 'salad', 'drinks']):
        return 'snacks'
